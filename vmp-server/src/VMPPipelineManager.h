...
*/
@interface VMPV4L2PipelineManager : VMPPipelineManager
...
/**
    @brief Pipeline manager for ALSA devices

    @property device The ALSA device to use

    This class is used to manage a pipeline for a given ALSA device.
    It creates and manages a GStreamer pipeline, with the ALSA device
    as source, and a interaudiosink with the given channel name as sink.

    The same device observation, and state handling as for V4L2 devices applies.
